...
*/
// FIX: Arrow (=>) harus di baris yang sama dengan parameter
...
export const getDraftTimestamp = (draftId = "create") => {
	try {
		const key = `${DRAFT_KEY_PREFIX}${draftId}`;
		// FIX: Nilai yang di-return harus di baris yang sama dengan 'return'
...
export const formatDraftTime = (timestamp) => {
	...
	if (diffMins < 60)
		return `${diffMins} menit yang 
lalu`;
	const diffHours = Math.floor(diffMins / 60);
	if (diffHours < 24)
		return `${diffHours} jam yang 
lalu`;
