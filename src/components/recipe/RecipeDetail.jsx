...
// BARU DITAMBAHKAN: Import ikon Share2
import {
	ArrowLeft,
	Heart,
	Clock,
...
export default function RecipeDetail({
	...
	// --- FUNGSI SHARE BARU ---
	// BARU DITAMBAHKAN: Fungsi untuk menangani share
	const handleShare = async () => {
		const shareData = {
			title: recipe?.name || "Resep Nusantara",
			text: `Lihat resep lezat ini: ${recipe?.name}`,
			// URL akan otomatis benar setelah di-deploy ke Vercel
				...
				<div className="grid md:grid-cols-2 gap-8 mb-8">
					...
					<div className="bg-white/60 backdrop-blur-sm rounded-3xl p-6 md:p-8 shadow-xl border border-white/40">
						<h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center gap-3">
							...
							>
								<span className={`text-${colors.primary}-600 text-xl`}>
									&#129378;
								</span>
							</div>
							Bahan-bahan
						...
						<ul className="space-y-3">
							{recipe.ingredients?.map((ingredient) => (
								...
								>
									<span className={`text-${colors.primary}-600 mt-1`}>
										&#10004;
									</span>
									<div>
